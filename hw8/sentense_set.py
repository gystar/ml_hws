...
class SentenseSet(torch.utils.data.Dataset):
    def __init__(self, path, word_dic: Dictionary):
        ...
        for line in open(path).readlines():
            ...
            line = line.strip("\n").split("\t")
            # 分词并将词语转换为one-hot编码
            en_codes = [word_dic.en_word2ix.get(c, UNK) for c in line[0].split(" ")[:-1]]  # 注意句子最后多了一个空格,因此会被多split出来一个空字符
            cn_codes = [word_dic.cn_word2ix.get(c, UNK) for c in line[1].split(" ")[:-1]]
            # 在开头和结尾加上标识符
        ...
        self.sentenses_en = torch.nn.utils.rnn.pad_sequence(self.sentenses_en, batch_first=True, padding_value=PAD)
        self.sentenses_cn = torch.nn.utils.rnn.pad_sequence(self.sentenses_cn, batch_first=True, padding_value=PAD)
        # pad之后句子的长度
