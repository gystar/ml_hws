...
LastEditTime: 2020-12-10 14:53:46
LastEditors: gystar
...
def activation2numpy(activation):
    # 将图片梯度转换为矩阵输出,#为方便起见，可将其先“标准化”再输出；方便画出对比图
    x = (activation - activation.min()) / (activation.max() - activation.min())
    return x.detach().cpu()
...
def visualization(images, model: torch.nn.Module, model_layer, filter_id, input_size, rtransformer, iters=10, lr=0.1):
    def hook(model, input, output):
        global g_output
        g_output = output  # 将激活后的结果通过全局变量传递出去

    hook_handle = model_layer.register_forward_hook(hook)  # hook到目标层
    input = torch.zeros(input_size).unsqueeze(0).to(images.device)
    ...
    activations = [activation2numpy(activation) for activation in activations]
    return rtransformer(image), activations
...
if __name__ == "__main__":
    ...
    model = torch.load("../hw3/<class 'image_classification.GYHF_LetNet5'>.pkl").to(device)
    images, lables = data.GetBatch([1, 3, 5])
    images = images.to(device)
    image, activations = visualization(
        images, model, model.features[0], 0, (3, model.input_size[0], model.input_size[1]), image_set.tensor2numpy, iters=1000, lr=0.01
    )
    plt.figure()
    plt.imshow(image)
