...
def generate(model, device, input_size, class_index, rtransformer, iters=10, lr=0.001):
    input = torch.zeros(input_size).unsqueeze(0).to(device)  # 从全0图片开始进行修正
    ...
    return rtransformer(input.squeeze(0))
...
if __name__ == "__main__":
    ...
    image = generate(model, device, (3, model.input_size[0], model.input_size[1]), 1, image_set.tensor2numpy, 1000, 0.01)
