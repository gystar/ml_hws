...
class GYHF_LetNet5(nn.Module):  # 继承父类
    ...
    def __init__(self, class_count):
        ...
        self.features = nn.Sequential(  # 使用nn.Sequential去定义特征
            nn.Conv2d(in_channels=3, out_channels=6, kernel_size=5),  # 6个5*5*3的filter  # (6,28,28)
...
class GYHF_VGG(nn.Module):  # 继承父类
    ...
    def __init__(
        self,
        num_classes: int,
        cfg: str = "vgg11",  # vgg11等，表示不同的VGG层级
...
class GYHF_TestNet(nn.Module):  # 继承父类
    ...
    def __init__(self, class_count):
        ...
        self.features = nn.Sequential(  # 使用nn.Sequential去定义特征
            nn.Conv2d(in_channels=3, out_channels=64, kernel_size=3),  # 64个5*5*3的filter  # (64,30,30)
            nn.BatchNorm2d(64),
            nn.ReLU(),  # Rectified Linear Unit
